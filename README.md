...
pnpm demo:source taskly-captions  # מקור שלישי: מסך מלא, בלי קריינות — רק כתוביות צרובות → input/short-captions.mp4
...
1. **ניתוח** — FFmpeg (מטא-דאטה, אודיו, scene detection, זרם תנועה ברזולוציה נמוכה), מעקב סמן ושינויי UI, דגימת פריימים חכמה, OCR (כולל מעבר ייעודי לכפתורים צבעוניים), תמלול.
   **סרטונים בלי קריינות:** כתוביות צרובות על המסך ("1. Click Team…") מזוהות כרצועה ממורכזת שהטקסט בה מתחלף, הופכות לתמלול מתוזמן, ומוסרות מה-OCR של הממשק (כדי שלא יהפכו לכפתור או לכותרת); שינויי תנועה בתוך רצועת הכתוביות לא נחשבים ללחיצות.
